...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const generatedText = await generateContent(file, outputType);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <main className="container mx-auto flex flex-col items-center gap-8 p-4 sm:p-8">
        <div className="w-full max-w-lg space-y-6 rounded-xl bg-white p-8 shadow-sm border border-gray-200">
            <h2 className="text-xl font-bold text-center text-gray-700">1. 音声ファイルをアップロード</h2>
            <FileUpload file={file} onFileChange={setFile} />
            <h2 className="text-xl font-bold text-center text-gray-700 pt-4">2. 出力タイプを選択</h2>
